<div id="rightMain" class="col-md-12" role="main">
    ...
    <form id="searchForm" role="form" action="" method="POST">
         <div class="row">
            <div class="col-md-2">
             ...
             <div class="col-md-2">
                 <input type="text" name="fileType" class="form-control" placeholder="日志类型" value="<?php echo $fileType;?>">
             </div>
             <div class="col-md-2">
    ...
    <table class="table table-hover">
        ...
        <tbody>
        <?php foreach($insertFileLogList as $insertFileLog):?>
            <tr>
                <td><?php echo $insertFileLog['date'];?></td>
                <td><?php echo $insertFileLog['pid'];?></td>
                <td><?php echo $insertFileLog['fileType'];?></td>
                <td><?php if($insertFileLog['endTime'] > 0) echo $insertFileLog['fileLine'];?></td>
                <td><?php if($insertFileLog['endTime'] > 0) echo round($insertFileLog['fileSize']/1024/1024, 2) . 'MB';?></td>
                <td><?php if($insertFileLog['endTime'] > 0) echo round($insertFileLog['memory']/1024/1024, 2) . 'MB';?></td>
                <td><?php echo date('H:i:s', $insertFileLog['startTime']);?></td>
                <td><?php if($insertFileLog['endTime'] > 0) echo date('H:i:s', $insertFileLog['endTime']);?></td>
                <td><?php if($insertFileLog['endTime'] > 0) echo ($insertFileLog['endTime'] - $insertFileLog['startTime']) . '秒'; else echo (time() - $insertFileLog['startTime']) . '秒';?></td>
                <td><?php if($insertFileLog['endTime'] > 0) echo '<span style="color: #008000">已完成</span>'; else echo '<span style="color: #ff0000">运行中</span>';?></td>
            </tr>
        <?php endforeach;?>
        </tbody>
    </table>
